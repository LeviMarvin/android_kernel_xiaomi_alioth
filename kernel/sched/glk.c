...
static bool force_maxfreq_break = false;
...
static unsigned int glk_highspeed_load[MAX_CLUSTERS] = {
        [0 ... MAX_CLUSTERS-1] = 90};
static unsigned int glk_lowspeed_load[MAX_CLUSTERS] = {
        [0 ... MAX_CLUSTERS-1] = 30};
...
static struct ctl_table_header *game_load_header = NULL;
...
#define for_each_cluster_cpumask(i, cpumask) \
	for(i = 0, cpumask = &cluster_cpumask[0]; \
		i < cluster_number; i++, cpumask = &cluster_cpumask[i])
...
int glk_enable(void)
{
  ...
  return (game_load_inuse() && (migt_enable() > 2)); /*enable in migt 3.0*/
}

static u64
game_runtime_update(struct game_runtime *load, u64 delta, bool new_frame)
{
...
void
game_load_update(struct task_struct *task, u64 delta, int cpu)
{
  ...
  for (i = 0; i < FREQ_LOAD_ITEMS; i++) {
    ...
    game_runtime_update(&per_cpu(load_info, cpu).load[i],
                        delta, 0);
  }
                           ...
                           unsigned long util, unsigned long max)
{
  ...
  if (unlikely((soft_maxfreq < soft_minfreq)
				|| !frame_period))
    return 0;
  ...
  target_freq = (glk_load * (soft_maxfreq + (soft_maxfreq >> 3))
			/ 100);
...
static u64 update_glk_load(struct cpumask *mask, int index, bool roll)
{
  ...
  for_each_cpu(cpu, mask) {
    for (i = 0; i < FREQ_LOAD_ITEMS; i++) {
      ...
      cpu_load[i] = game_runtime_update(cpu_runtime,
                                        0, roll);
...
static int get_target_period(int migt_thresh)
{
  ...
  if (likely(old_migt_thresh == migt_thresh)
			&& frame_tperiod)
...
void game_load_history_update(u64 tick)
{
  ...
  if (likely(cur_frame_load->frame_start)) {
    ...
    cpu_util_freq_divisor =
        get_target_period(migt_thresh) << 10;
...
void create_game_load_proc(void *rootdir)
{
  ...
  history_load_entry = proc_create("game_history_load",
                                   0664, game_load_dir, &history_load_fops);
  cpu_load_entry = proc_create("game_cpu_load", 0664,
                               game_load_dir, &cpu_load_fops);
}
...
/* sysctl interface */
int proc_glk_dointvec(struct ctl_table *table, int write,
                      void __user *buffer, size_t *lenp,
                      loff_t *ppos, glk_dointvec_f f)
{
  int ret;
...
static int proc_glk_dotload(struct ctl_table *table, int write,
                            ...
                            loff_t *ppos)
{
  int i, ret = proc_glk_dointvec(table, write, buffer, lenp, ppos,
                                 NULL);
  ...
  for (i = 0; i < MAX_CLUSTERS; i++)
    pr_info("cid %d minload %d maxload %d\n",
            i, glk_lowspeed_load[i],
            glk_highspeed_load[i]);
  return ret;
}
...
static int proc_glk_dofreq(struct ctl_table *table, int write,
                           ...
                           loff_t *ppos)
{
  int i, ret = proc_glk_dointvec(table, write, buffer,
                                 lenp, ppos, NULL);
...
int game_load_init(void)
{
  ...
  for_each_cpu(i, &cpus) {
    ...
    for_each_cpu(cpu, cluster_cpus) {
      ...
      raw_spin_lock_init(&per_cpu(load_info, cpu).loading_slock);
    }
