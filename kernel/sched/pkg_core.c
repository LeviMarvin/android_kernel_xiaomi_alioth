...
static void package_runtime_roll_wk(struct work_struct *work)
{
  ...
  for_each_set_bit(bit, package_uid_bit,
    ...
    if (user) {
      ...
      user_zero.pkg.little_cluster_runtime[curr_runtime_items] =
          user_zero.pkg.little_cluster_runtime[HISTORY_ITMES];
      for(i = 0; i < MAX_CLUSTER; i++) {
        user_zero.pkg.back_runtime[curr_runtime_items][i] =
            user_zero.pkg.back_runtime[HISTORY_ITMES][i];
        user_zero.pkg.front_runtime[curr_runtime_items][i] =
            user_zero.pkg.front_runtime[HISTORY_ITMES][i];
      }
    }
...
static void update_task_runtime_info(struct task_struct *tsk, u64 delta, int cpu)
{
  ...
  case FRONT:
    ...
    tsk->pkg.front_runtime[curr_items][clustertype] = tsk->pkg.front_runtime[HISTORY_ITMES][clustertype];
    user->pkg.front_runtime[HISTORY_ITMES][clustertype] += delta;

    break;
  case BACK:
    tsk->pkg.back_runtime[HISTORY_ITMES][clustertype] += delta;
    tsk->pkg.back_runtime[curr_items][clustertype] = tsk->pkg.back_runtime[HISTORY_ITMES][clustertype];
  ...
  tsk->pkg.sup_cluster_runtime[curr_items] = tsk->pkg.sup_cluster_runtime[HISTORY_ITMES];
  tsk->pkg.big_cluster_runtime[curr_items] = tsk->pkg.big_cluster_runtime[HISTORY_ITMES];
  tsk->pkg.little_cluster_runtime[curr_items] = tsk->pkg.little_cluster_runtime[HISTORY_ITMES];
