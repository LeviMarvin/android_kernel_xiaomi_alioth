...
static int runtime_show_all(struct seq_file *m, void *v)
{
  ...
  kuid_t uid;
  int i, bit;
  int last_uid = MIN(*((int*) get_uid_max_value()) + 1, TRACE_UID_MAX-1);
  ...
  seq_printf(m, "time info: from [%llu]s to [%llu]s ago\n",
             (get_jiffies_64()
			- package_runtime_info[prev_index].update_time) / HZ,
             (get_jiffies_64()
			- package_runtime_info[pprev_index].update_time) / HZ);
  ...
  for_each_set_bit(bit, package_uid_bit, last_uid) {
    ...
    if (app_run_on_scluster + app_run_on_bcluster
			+ app_run_on_lcluster > top_package_load[TOP_ALL]) {
      /*top load package*/
      top_package_load[TOP_ALL] = app_run_on_scluster
				+ app_run_on_bcluster + app_run_on_lcluster;
      top_package_uid[TOP_ALL] = bit;
    }

    bcluster_bias = (int)((user->pkg.big_cluster_runtime[prev_index]
			- user->pkg.big_cluster_runtime[pprev_index])
			* 10000 / package_runtime);
    if (bcluster_bias < 0)
      bcluster_bias = 0;

    scluster_bias = (int)((user->pkg.sup_cluster_runtime[prev_index]
			- user->pkg.sup_cluster_runtime[pprev_index])
			* 10000 / package_runtime);
    if (scluster_bias < 0)
      scluster_bias = 0;

    scluster_total_usage += (user->pkg.sup_cluster_runtime[prev_index] -
                             user->pkg.sup_cluster_runtime[pprev_index]);
    bcluster_total_usage += (user->pkg.big_cluster_runtime[prev_index] -
                             user->pkg.big_cluster_runtime[pprev_index]);
    lcluster_total_usage += (user->pkg.little_cluster_runtime[prev_index]
			- user->pkg.little_cluster_runtime[pprev_index]);
    ...
    seq_printf(m,
               ...
               scluster_total_usage, bcluster_total_usage, lcluster_total_usage);

    seq_printf(m, "top load package info--- 0 for s/b/l cluster\n");
    ...
    static int package_runtime_show(struct seq_file *m, void *v)
{
      ...
      list_for_each(l, &(user->pkg.list)) {
        ...
        app_run_on_scluster = (int)(app_run_stime
			* 10000 / delta_exec);
        if (app_run_on_scluster > 0)
          scluster_usage += app_run_on_scluster;
        else
        ...
        app_run_on_bcluster = (int)(app_run_btime
			* 10000 / delta_exec);
        if (app_run_on_bcluster > 0)
          bcluster_usage += app_run_on_bcluster;
        else
        ...
        app_run_on_lcluster = (int)(app_run_ltime
			* 10000 / delta_exec);
        if (app_run_on_lcluster > 0)
          lcluster_usage += app_run_on_lcluster;
        else
        ...
        scluster_bias = (int)(app_run_stime * 10000
			/ (app_run_stime + app_run_btime + app_run_ltime));
        if (scluster_bias < 0)
          scluster_bias = 0;
        bcluster_bias = (int)(app_run_btime * 10000
			/ (app_run_stime + app_run_btime + app_run_ltime));
        if (bcluster_bias < 0)
          bcluster_bias = 0;
        ...
        seq_printf(m,
                   ...
                   p->pid, p->comm, app_run_stime, app_run_on_scluster/100,
                   app_run_on_scluster%100,
      ...
      seq_printf(m, "usage total sup:[%d.%d] big:[%d.%d] little:[%d.%d] \n",
                 scluster_usage/100,
                 scluster_usage%100, bcluster_usage/100,
      ...
      static int fb_show_all(struct seq_file *m, void *v)
{
        ...
        seq_printf(m, "time delta: [%llu]s, UNIT is ms!!\n",
                   (get_jiffies_64() - package_runtime_info[fbindex].update_time)/HZ);
        ...
        for_each_set_bit(bit, package_uid_bit, last_uid) {
          ...
          for (i = 0; i < MAX_CLUSTER; i++) {
            front_usage += (user->pkg.front_runtime[prev_index][i] - user->pkg.front_runtime[pprev_index][i]);
            back_usage += (user->pkg.back_runtime[prev_index][i] - user->pkg.back_runtime[pprev_index][i]);
          }
          ...
          seq_printf(m,
                     "%5d: <ALL>%16llu <FRONT>%16llu <BACK>%16llu\n",
                     bit, (front_usage + back_usage)/NSEC_PER_MSEC,
                     front_usage/NSEC_PER_MSEC, back_usage/NSEC_PER_MSEC);

          seq_printf(m,"<FRONT>:<l>%16llu, <b>%16llu, <s>%16llu\n",
                     (user->pkg.front_runtime[prev_index][0]
			- user->pkg.front_runtime[pprev_index][0])/NSEC_PER_MSEC,
                     (user->pkg.front_runtime[prev_index][1]
			- user->pkg.front_runtime[pprev_index][1])/NSEC_PER_MSEC,
                     (user->pkg.front_runtime[prev_index][2]
			- user->pkg.front_runtime[pprev_index][2])/NSEC_PER_MSEC);

          seq_printf(m,"<BACK>:<l>%16llu, <b>%16llu, <s>%16llu\n",
                     (user->pkg.back_runtime[prev_index][0]
			- user->pkg.back_runtime[pprev_index][0])/NSEC_PER_MSEC,
                     (user->pkg.back_runtime[prev_index][1]
			- user->pkg.back_runtime[pprev_index][1])/NSEC_PER_MSEC,
                     (user->pkg.back_runtime[prev_index][2]
			- user->pkg.back_runtime[pprev_index][2])/NSEC_PER_MSEC);
          free_uid(user);
        }
        ...
        seq_printf(m,
                   ...
                   all_front_usage/NSEC_PER_MSEC, all_back_usage/NSEC_PER_MSEC);
      ...
      static int fb_show_uid(struct seq_file *m, void *v)
{
        ...
        for_each_process(tsk) {
          ...
          for_each_thread(tsk, p) {
            ...
            back_usage = 0;
            for (i = 0; i < MAX_CLUSTER; i++) {
              front_usage += (p->pkg.front_runtime[prev_index][i]
					- p->pkg.front_runtime[pprev_index][i]);
              back_usage += (p->pkg.back_runtime[prev_index][i]
            ...
            seq_printf(m,
                       ...
                       p->pid, p->comm, (front_usage + back_usage)/NSEC_PER_MSEC,
                       front_usage/NSEC_PER_MSEC, back_usage/NSEC_PER_MSEC);
            seq_printf(m,"<FRONT>: <l>%16llu, <b>%16llu, <s>%16llu\n",
                       (p->pkg.front_runtime[prev_index][0]
				- p->pkg.front_runtime[pprev_index][0])/NSEC_PER_MSEC,
                       (p->pkg.front_runtime[prev_index][1]
				- p->pkg.front_runtime[pprev_index][1])/NSEC_PER_MSEC,
                       (p->pkg.front_runtime[prev_index][2]
				- p->pkg.front_runtime[pprev_index][2])/NSEC_PER_MSEC);
            seq_printf(m,"<BACK>: <l>%16llu, <b>%16llu, <s>%16llu\n",
                       (p->pkg.back_runtime[prev_index][0]
				- p->pkg.back_runtime[pprev_index][0])/NSEC_PER_MSEC,
                       (p->pkg.back_runtime[prev_index][1]
				- p->pkg.back_runtime[pprev_index][1])/NSEC_PER_MSEC,
                       (p->pkg.back_runtime[prev_index][2]
				- p->pkg.back_runtime[pprev_index][2])/NSEC_PER_MSEC);
          }
        }

        seq_printf(m,
                   "%5d: <ALL>%16llu <FRONT>%16llu <BACK>%16llu\n",
                   traced_uid, (all_front_usage + all_back_usage)/NSEC_PER_MSEC,
      ...
      static int fb_open_uid(struct inode *inode, struct file *file)
{
      ...
      static ssize_t fb_uid_write(struct file *filp,
                                  const char __user *buf, size_t count, loff_t *f_ops)
{
        unsigned char buffer[32] = {0};
        int value = 0;
      ...
      static int pkg_condition_reset(struct seq_file *m, void *v)
{
        ...
        seq_printf(m, "font&back stats reset success\n");
        return 0;
      }
      ...
      static int reset_traced_window_show(struct seq_file *m, void *v)
{
        ...
        seq_printf(m, "reset traced window show success!\n");
        return 0;
      }
      ...
      void create_runtime_proc(void)
{
        ...
        package_showall_entry =
            proc_create("show_all", 0664,
                        package_dir, &package_show_all_fops);
        package_top_bcore_entry =
            proc_create("top_package_on_bcore", 0664,
                        package_dir, &package_top_bcore_fops);
        package_top_lcore_entry =
            proc_create("top_package_on_lcore", 0664,
                        package_dir, &package_top_lcore_fops);
        package_top_entry =
            proc_create("top_package", 0664,
                        package_dir, &package_top_fops);
        package_trace_entry =
            proc_create("show_traced_package", 0664,
                        package_dir, &package_trace_fops);
        package_fb_showall =
            proc_create("show_fb_showall", 0666,
                        package_dir, &package_fb_fops);
        package_fb_traced_show =
            proc_create("show_fb_uid_show", 0666,
        ...
        package_tracestat_entry =
            proc_create("show_tracestat", 0664,
                        stat_dir, &package_tracestat_fops);
        package_trace_window =
            proc_create("show_traced_window", 0664,
                        stat_dir, &package_tracedwindow_fops);
        package_window_size_entry =
            proc_create("show_windowsize", 0664,
                        stat_dir, &package_windowsize_fops);
        package_develop_mode_entry =
            proc_create("develop_mode", 0664,
                        stat_dir, &develop_mode_fops);
        package_resettraced_window =
            proc_create("reset_traced_window", 0664,
                        stat_dir, &package_reset_tracedwindow_fops);
        package_condition_reset =
            proc_create("pkg_condition_reset", 0666,
                        stat_dir, &pkg_cond_reset_fops);
        package_fb_status_set =
            proc_create("pkg_fb_set", 0666,
                        stat_dir, &pkg_fb_set_fops);
        package_pause_mode_entry =
            proc_create("pause_mode", 0664,
                        stat_dir, &pause_mode_fops);
        ...
        exec_buckets_entry =
            proc_create("show_exec_buckets", 0664,
                        migt_dir, &exec_buckets_fops);
