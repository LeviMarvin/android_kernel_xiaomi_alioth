...
int __weak vip_task_schedboost = 0;
...
int __weak migt_enable(void) {return 0; }
...
void migt_monitor_hook(int enqueue, int cpu,
                       struct task_struct *p, u64 walltime)
{
  ...
  if (enqueue)
    ...
    exec_delta = p->se.sum_exec_runtime -
                 p->pkg.migt.prev_sum;
...
static
bool is_render_sibling(struct task_struct *tsk, struct rend_info_s *rinfo)
{
  int type, len;
...
static void set_viptask_flag(struct task_struct *p, int pri, int super_id)
{
  if (pri == -1) {
    ...
    p->pkg.migt.flag |= 1 << GAME_SUPER_TASK;
    if (migt_debug)
      printk("set %s pri %d as super\n", p->comm, pri);
    return;
  } else if (pri < stask_candidate_num + vip_task_max_num) {
    if (migt_debug)
      printk("set %s pri %d as vip\n", p->comm, pri);
    p->pkg.migt.flag |= 1 << GAME_VIP_TASK;
    return;
  } else if (pri < stask_candidate_num
		    + vip_task_max_num + ip_task_max_num)
...
static void mark_viptask(int uid)
{
  ...
  for_each_user_process(tsk, user) {
    ...
    if (need_reset_runtime_everytime
			|| force_reset_runtime) {
  ...
  if (i > 0) {
    ...
    if (pick_stask_policy == CHOOSE_HERAY)
      super_id = pick_stask(user_array,
                            min_value(stask_candidate_num, i));
...
int proc_viptask_cpus_set(struct ctl_table *table, int write,
                          void __user *buffer, size_t *lenp, loff_t *ppos)
{
  int cpu, ret = proc_dointvec(table, write,
                               buffer, lenp, ppos);
...
static int __init migt_sched_init(void)
{
  ...
  /*run thread*/
  migt_thread =
      kthread_run(migt_hardwork, &g_work, MIGT_THREAD_NAME);
...
int game_task(struct task_struct *p)
{
  ...
  return  (from_kuid(&init_user_ns, task_uid(p))
			== g_work.uid);
}
  ...
  for_each_cpu(i, &cpus) {
    ...
    if (clus_affinity < CLUSTER_AFFINITY_TYPES) {
      ...
      cpumask_copy(&cluster_affinity_cpumask[clus_affinity],
                   &tcpus);
      for_each_cpu(cpu, &cluster_affinity_cpumask[clus_affinity])
          pr_info("-%d \t", cpu);
      pr_info("\n");
    }
    clus_affinity++;
...
void update_render_info(struct task_struct *tsk, enum RENDER_TYPE type)
{
  ...
  memset(render_info[type].comm, 0, TASK_COMM_LEN);
  len = strlen(tsk->comm);
  strncpy(render_info[type].comm, tsk->comm, len);
  write_seqcount_end(&render_info[type].seq);
  raw_spin_unlock(&render_info[type].lock);
