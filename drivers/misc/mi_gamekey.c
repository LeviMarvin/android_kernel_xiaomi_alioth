...
static unsigned int gamekey_poll(struct file *file, poll_table *wait)
{
	...
	pr_info("poll hall event, mask = %d\n", mask);
	return mask;
}
...
static int parse_dt(struct gamekey_core *core)
{
	...
	pr_info("hall left gpio = %d\n",  core->hw.gpio_info[HALL_LEFT].gpio);
	pr_info("hall right gpio = %d\n", core->hw.gpio_info[HALL_RIGHT].gpio);
	pr_info("key left gpio = %d\n",  core->hw.gpio_info[GAMEKEY_LEFT].gpio);
	pr_info("key right gpio = %d\n",   core->hw.gpio_info[GAMEKEY_RIGHT].gpio);
	pr_info("debounce time = %d\n", core->hw.debounce_time);
...
static int input_init(struct gamekey_core *core)
{
	...
	if (ret) {
		pr_err("input register failed, ret=%d\n", ret);
		goto input_reg_err;
	}
...
static irqreturn_t key_left_handler(int irq, void *data)
{
	...
	pr_debug("%s, sts = %d\n", __func__, core->sts[GAMEKEY_LEFT]);
	return IRQ_HANDLED;
}
...
static irqreturn_t key_right_handler(int irq, void *data)
{
	...
	pr_debug("%s, sts = %d\n", __func__, core->sts[GAMEKEY_RIGHT]);
	return IRQ_HANDLED;
}
...
static int gpio_init(struct gamekey_core *core)
{
	...
	if (ret) {
		pr_err("pinctrl select failed, ret = %d\n", ret);
...
static int irq_init(struct gamekey_core *core)
{
	...
	if ((hw->hall_l_irq > 0) && (hw->hall_r_irq > 0) &&
		...
		ret = request_threaded_irq(hw->hall_l_irq, NULL, hall_left_handler,
				...
				"hall_l_irq", core);
		if (ret) {
			pr_err("request irq handler for hall left failed, ret = %d\n", ret);
			goto hl_err;
		}
		...
		ret = request_threaded_irq(hw->hall_r_irq, NULL, hall_right_handler,
				...
				"hall_r_irq", core);
		if (ret) {
			pr_err("request irq handler for hall right failed, ret = %d\n", ret);
			goto hr_err;
		}
		...
		ret = request_threaded_irq(hw->key_l_irq, NULL, key_left_handler,
				...
				"key_l_irq", core);
		if (ret) {
			pr_err("request irq handler for key left failed, ret = %d\n", ret);
			goto kl_err;
		}
		...
		ret = request_threaded_irq(hw->key_r_irq, NULL, key_right_handler,
				...
				"key_r_irq", core);
		if (ret) {
			pr_err("request irq handler for key right failed, ret = %d\n", ret);
			goto kr_err;
		}

	} else {
		pr_err("irq number illegal, hall left: %d, hall right: %d, key left: %d, key right: %d\n",
